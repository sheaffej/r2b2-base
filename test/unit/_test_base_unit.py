...
from r2b2_base.odometry_helpers import heading_from_odometry
from roboclaw_interfaces.msg import Stats

# class MockBaseNode():
#     def __init__(self):
#         self._last_cmd_vel_time = None
#         self._deadman_secs = 1
#         self._x_linear_cmd = 0.0
#         self._z_angular_cmd = 0.0
#         self._cmd_vel_lock = RLock()
...
#         self._max_drive_secs
#         self._max_qpps
#         self._max_accel

#         self._speed_cmd_pub

#         self._roboclaw_front_stats
...
def test_start_zero(speed_cmd_pub: MockSpeedCmdPub, odom_pub: MockOdomPub):
    self.base._last_cmd_vel_time = None
    self.base.process_base_loop()
    _compare_speed_command(speed_cmd_pub.msg, 0, 0)
    _compare_odometry(odom_pub.msg, 0, 0, 0, 0, 0, 0)
...
# Helper methods
#
def _compare_odometry(odom, exp_world_x, exp_world_y, exp_world_theta,
...
def _compare_speed_command(speed_cmd_msg, exp_m1_qpps, exp_m2_qpps):
    print("Stats:")
...
def _send_stats(m1_enc_val, m2_enc_val):
    stats = Stats()
...
def _send_cmd_vel(linear_x, angular_z):
    twist = Twist()
